...
//Copy from https://github.com/moonlight-stream/moonlight-android/blob/master/app/src/main/java/com/limelight/binding/video/MediaCodecHelper.java
public class MediaCodecHelper {
    ...
    private static String getAdrenoVersionString(String glRenderer) {
        ...
        Log.e("Easycontrol", "Found Adreno GPU: "+modelNumber);
        return modelNumber;
    }
    ...
    public static void initialize(Context context, String glRenderer) {
        ...
        if (configInfo.reqGlEsVersion != ConfigurationInfo.GL_ES_VERSION_UNDEFINED) {
            Log.e("Easycontrol", "OpenGL ES version: "+configInfo.reqGlEsVersion);
            ...
            if (configInfo.reqGlEsVersion >= 0x30000) {
                Log.e("Easycontrol", "Added omx.nvidia/c2.nvidia to reference frame invalidation support list");
                ...
                Log.e("Easycontrol", "Added omx.qcom/c2.qti to reference frame invalidation support list");
            ...
            if (isGLES31SnapdragonRenderer(glRenderer)) {
                Log.e("Easycontrol", "Added omx.qcom/c2.qti to HEVC decoders based on GLES 3.1+ support");
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N && isPowerVR(glRenderer)) {
                Log.e("Easycontrol", "Added omx.mtk to HEVC decoders based on PowerVR GPU");
                whitelistedHevcDecoders.add("omx.mtk");
                ...
                if (glRenderer.contains("GX6")) {
                    Log.e("Easycontrol", "Added omx.mtk/c2.mtk to RFI list for HEVC");
                    refFrameInvalidationHevcPrefixes.add("omx.mtk");
                    refFrameInvalidationHevcPrefixes.add("c2.mtk");
                }
            }
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.R) {
            try {
                if (decoderInfo.getCapabilitiesForType(mimeType).isFeatureSupported(CodecCapabilities.FEATURE_LowLatency)) {
                    Log.e("Easycontrol", "Low latency decoding mode supported (FEATURE_LowLatency)");
                    return true;
                }
            } catch (Exception e) {
                ...
                for (String supportedOption : testCodec.getSupportedVendorParameters()) {
                    for (String knownLowLatencyOption : knownVendorLowLatencyOptions) {
                        if (supportedOption.equalsIgnoreCase(knownLowLatencyOption)) {
                            Log.e("Easycontrol", decoderName + " supports known low latency option: " + supportedOption);
                            return true;
                        }
                    }
                }
    ...
    public static boolean decoderSupportsFusedIdrFrame(MediaCodecInfo decoderInfo, String mimeType) {
        ...
        try {
            if (decoderInfo.getCapabilitiesForType(mimeType).
                ...
                Log.e("Easycontrol", "Decoder supports fused IDR frames (FEATURE_AdaptivePlayback)");
                return true;
            }
        } catch (Exception e) {
            // Tolerate buggy codecs
    ...
    public static boolean decoderSupportsAdaptivePlayback(MediaCodecInfo decoderInfo, String mimeType) {
        if (isDecoderInList(blacklistedAdaptivePlaybackPrefixes, decoderInfo.getName())) {
            Log.e("Easycontrol", "Decoder blacklisted for adaptive playback");
            return false;
        }
        ...
        try {
                    ...
                    isFeatureSupported(CodecCapabilities.FEATURE_AdaptivePlayback))
            {
                ...
                Log.e("Easycontrol", "Adaptive playback supported (FEATURE_AdaptivePlayback)");
                return true;
            }
        } catch (Exception e) {
            // Tolerate buggy codecs
    ...
    public static boolean decoderSupportsRefFrameInvalidationHevc(MediaCodecInfo decoderInfo) {
        ...
        if (decoderSupportsAndroidRLowLatency(decoderInfo, "video/hevc") ||
            ...
            Log.e("Easycontrol", "Enabling HEVC RFI based on low latency option support");
            return true;
        }
    ...
    public static boolean decoderSupportsRefFrameInvalidationAv1(MediaCodecInfo decoderInfo) {
        ...
        if (decoderSupportsAndroidRLowLatency(decoderInfo, "video/av01") ||
                decoderSupportsKnownVendorLowLatencyOption(decoderInfo.getName())) {
            Log.e("Easycontrol", "Enabling AV1 RFI based on low latency option support");
            return true;
        }
    ...
    public static boolean decoderIsWhitelistedForHevc(MediaCodecInfo decoderInfo) {
        ...
        if (decoderInfo.getName().contains("sw")) {
            Log.e("Easycontrol", "Disallowing HEVC on software decoder: " + decoderInfo.getName());
            return false;
        }
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.S) {
            Log.e("Easycontrol", "Media performance class: " + Build.VERSION.MEDIA_PERFORMANCE_CLASS);
            if (Build.VERSION.MEDIA_PERFORMANCE_CLASS >= Build.VERSION_CODES.S) {
                Log.e("Easycontrol", "Allowing HEVC based on media performance class");
                return true;
            }
        }
        ...
        if (decoderSupportsAndroidRLowLatency(decoderInfo, "video/hevc")) {
            Log.e("Easycontrol", "Allowing HEVC based on FEATURE_LowLatency support");
            return true;
        }
    ...
    public static boolean isDecoderWhitelistedForAv1(MediaCodecInfo decoderInfo) {
        ...
        //
        if (decoderInfo.getName().contains("sw")) {
            Log.e("Easycontrol", "Disallowing AV1 on software decoder: " + decoderInfo.getName());
            return false;
        }
        ...
        for (String preferredDecoder : preferredDecoders) {
            for (MediaCodecInfo codecInfo : getMediaCodecList()) {
                ...
                if (preferredDecoder.equalsIgnoreCase(codecInfo.getName())) {
                    Log.e("Easycontrol", "Preferred decoder choice is "+codecInfo.getName());
                    return codecInfo;
                }
            }
        }
    ...
    private static boolean isCodecBlacklisted(MediaCodecInfo codecInfo) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) {
            if (!SHOULD_BYPASS_SOFTWARE_BLOCK && codecInfo.isSoftwareOnly()) {
                Log.e("Easycontrol", "Skipping software-only decoder: "+codecInfo.getName());
                return true;
            }
        }
        ...
        if (isDecoderInList(blacklistedDecoderPrefixes, codecInfo.getName())) {
            Log.e("Easycontrol", "Skipping blacklisted decoder: "+codecInfo.getName());
            return true;
        }
        ...
        for (MediaCodecInfo codecInfo : getMediaCodecList()) {
            ...
            for (String mime : codecInfo.getSupportedTypes()) {
                if (mime.equalsIgnoreCase(mimeType)) {
                    ...
                    Log.e("Easycontrol", "First decoder choice is "+codecInfo.getName());
                    return codecInfo;
                }
            }
        }
    ...
    private static MediaCodecInfo findKnownSafeDecoder(String mimeType, int requiredProfile) throws Exception {
        ...
        for (int i = 0; i < 2; i++) {
            for (MediaCodecInfo codecInfo : getMediaCodecList()) {
                ...
                for (String mime : codecInfo.getSupportedTypes()) {
                    if (mime.equalsIgnoreCase(mimeType)) {
                        Log.e("Easycontrol", "Examining decoder capabilities of " + codecInfo.getName() + " (round " + (i + 1) + ")");
                        ...
                        if (i == 0 && !decoderSupportsAndroidRLowLatency(codecInfo, mime)) {
                            Log.e("Easycontrol", "Skipping decoder that lacks FEATURE_LowLatency for round 1");
                            continue;
                        }
                        ...
                        if (requiredProfile != -1) {
                            for (CodecProfileLevel profile : caps.profileLevels) {
                                if (profile.profile == requiredProfile) {
                                    Log.e("Easycontrol", "Decoder " + codecInfo.getName() + " supports required profile");
                                    return codecInfo;
                                }
                            }

                            Log.e("Easycontrol", "Decoder " + codecInfo.getName() + " does NOT support required profile");
                        } else {
                            return codecInfo;
                        }
                    }
    ...
    public static boolean isExynos4Device() {
        try {
            ...
            if (stringContainsIgnoreCase(cpuInfo, "SMDK4")) {
                Log.e("Easycontrol", "Found SMDK4 in /proc/cpuinfo");
                return true;
            }

            // If we see "Exynos 4" also we'll count it
            if (stringContainsIgnoreCase(cpuInfo, "Exynos 4")) {
                Log.e("Easycontrol", "Found Exynos 4 in /proc/cpuinfo");
                return true;
            }
        } catch (Exception e) {
            e.printStackTrace();
            ...
            if (files != null) {
                for (File f : files) {
                    if (stringContainsIgnoreCase(f.getName(), "exynos4")) {
                        Log.e("Easycontrol", "Found exynos4 in /sys/devices/system");
                        return true;
                    }
                }
            }
